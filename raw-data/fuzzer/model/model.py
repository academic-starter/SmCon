...
from .keycalculator import toInt, toUInt, getArrayBeginingSlot, getKeyisNonValueTypeSlotCalculation, getKeyisValueTypeSlotCalculation
...
class VariableModel:
    ...
    #   66-2*(self.offset+self.numOfBytes):66-2*self.offset
        ...
        if TypeParser.parse(self.varType) == UINT:
            ...
            self.varValue = toUInt(value)
        elif TypeParser.parse(self.varType) == INT:
            self.varValue =  toInt(value)  
        elif TypeParser.parse(self.varType) in [BYTE, BYTES]:
            self.varValue = value 
        elif TypeParser.parse(self.varType) == ADDRESS:
            ...
            self.varValue = True if toUInt(value) == 1 else False
        else:
...
class StringVariableModel(VariableModel):
    ...
    def beginingSlot(self):
        return getArrayBeginingSlot(self.varSlot) if self.byteSize>31 else self.varSlot
...
class MappingVariableModel(VariableModel):
    ...
    def implyAndSet(self, slot: int, value: Any):
        global TURN_ON_PARRALLEL 

        # if TURN_ON_PARRALLEL or len(GLOBAL_INNER_KEYS) < 50:
        # This code is not efficient
        for guess_key in GLOBAL_INNER_KEYS:
                if self.tryImplyAndSet(slot, value, guess_key):
                        return True 
        # else:
        #     TURN_ON_PARRALLEL = True
        #     # Use multi-core advantages 
        #     batch_size = 100
           
        #     for i in range(0, len(GLOBAL_INNER_KEYS), batch_size):
        #         with multiprocessing.Pool(int(multiprocessing.cpu_count()/2)) as pool:
        #             keys = GLOBAL_INNER_KEYS[i: min(i+batch_size, len(GLOBAL_INNER_KEYS))]
        #             successes = pool.starmap(tryImplyAndSet, zip(repeat(self), repeat(slot), repeat(value), keys))
        #             if any(successes):
        #                 TURN_ON_PARRALLEL = False
        #                 return True
        #     TURN_ON_PARRALLEL = False
    ...
    def beginingSlot(self, key: Any):
        matchedSlotHex =  getKeyisNonValueTypeSlotCalculation(key, self.varSlot)  \
            if isinstance(self.key_var_type, StringVariableModel) else \
            getKeyisValueTypeSlotCalculation(key, self.varSlot) 
...
class DynamicArrayVariableModel(ArrayVariableModel):
    ...
    def beginingSlot(self):
        return getArrayBeginingSlot(self.varSlot)
