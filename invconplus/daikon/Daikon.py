...
def print_function_decl(contractStorageObject, function, contractName):
    ...
    for item in function["inputs"]:
        if not item["type"].find("[") != -1:
            ...
            varname = item["name"]
            decls.append(_var_decl(varname, "hashcode",
                         "hashcode", Comparability_Indice))
            decls.append(_var_decl(varname, vartype, reptype,
                         comparability, isArray=True))
    #
    decls.append(_var_decl(varname="msg.sender", vartype="address",
                 reptype="java.lang.String", comparability=Comparability_Address))
    decls.append(_var_decl(varname="msg.value", vartype="uint",
                 reptype="java.math.BigInteger", comparability=Comparability_Int))
    decls.append(_var_decl(varname="block.timestamp", vartype="uint",
                 reptype="java.math.BigInteger", comparability=Comparability_Int))

    fullfuncname = _get_full_function_name(
...
def getDecl(contractName, contractStorageObject, contractAbi=list()):
    ...
    for function in contractAbi:
        if function["type"] == "function" and ("stateMutability" not in function or function["stateMutability"] != "view"):
            # print(function)
            enter, exit1, exit2 = print_function_decl(
                contractStorageObject, function, contractName)
...
def inferLikelyInvaraints(data_trace_file, invariant_file):
    cmd = f"""export DAIKONDIR=./InvCon/daikon-5.8.6
...
def isPredicate(inv):
    ...
    if symbol != "==" and re.match(r'^orig\(.*\)$', left) is not None and \
        ...
        hasParameter = (re.match(r'^orig\(this\..*\)$', left)
                        is None) or (re.match(r'orig\(this\..*\)', right) is None)
        if hasParameter:
            return True
        else:
            return False
...
def filterOutInterestingInvariants(invariant_files, contractInvariants):
    ...
    for inv_file in invariant_files:
        lines = open(inv_file).read().split("===========================================================================")[
            1].strip().split("Exiting Daikon.")[0].splitlines()
        ...
        # lines = lines[1:]
        lines = list(filter(lambda line:  line.find("block.timestamp")
                     == -1 and line.find("null") == -1, lines[1:]))
    ...
    for func_head in all_file_invariants.keys():
        ...
        for i in range(len(invariant_files)):
            ...
            if invariant_files[i].find("eq_") != -1:
                eq_inv = "" + \
                    invariant_files[i].split("eq_")[1].split(".inv")[
                        0].strip()+" == user"
            updateInvs = list(
                filter(lambda line: isUpdateFunction(line), invs))
            ...
            if config.DEBUG:
                ...
                print(propertyInvs)
                print("")
            file_intersting_invariants[func_head].append(
                [updateInvs, predicateInvs, propertyInvs])
...
def filterOutERC20Invariants(invariant_files):
    ...
    for inv_file in invariant_files:
        ...
        func_invs = open(inv_file).read().split("Exiting Daikon.")[0].split(
            "===========================================================================")[1:]
        for func_inv in func_invs:
            func_inv = func_inv.strip()
            func_head = func_inv.splitlines()[0].strip()
            lines = list(filter(lambda line: line.find("block.timestamp")
                         == -1 and line.find("null") == -1, func_inv.splitlines()[1:]))
            updateInvs = list(
                filter(lambda line: isUpdateFunction(line), lines))
